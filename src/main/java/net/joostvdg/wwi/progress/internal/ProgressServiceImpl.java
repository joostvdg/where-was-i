...
public class ProgressServiceImpl implements ProgressService {
  ...
  public Set<Progress> getProgressForUserAndMedia(int userId, Set<Media> items) {
    ...
    Result<AllProgressRecord> userProgressRecords =
        create
  ...
  public void createMovieProgressForUser(User user, Movie movie) {
    ...
    create
        .insertInto(Tables.MOVIE_PROGRESS)
  ...
  public void createSeriesProgressForUser(User user, Series series) {
    ...
    create
        .insertInto(Tables.SERIES_PROGRESS)
  ...
  public void createVideoGameProgressForUser(User user, VideoGame videoGame) {
    logger.info(
        "Creating Progress for User {} and VideoGame {}", user.username(), videoGame.getTitle());
    String progressJson = genereateDefaultProgressJSONForVideoGame();

    create
        .insertInto(Tables.VIDEO_GAME_PROGRESS)
  ...
  private void updateVideoGameProgress(VideoGameProgress updatedProgress) {
    ...
    create
        .update(Tables.VIDEO_GAME_PROGRESS)
        .set(Tables.VIDEO_GAME_PROGRESS.PROGRESS, JSONB.valueOf(progressJSON))
  ...
  private Progress translateRecordToProgress(AllProgressRecord rec) {
    ...
    switch (rec.getMediaType()) {
      case Movie.TYPE:
        // int id, String title, String platform,String director,int durationInMinutes,int
        // releaseYear,Set<String> genre,Optional<String> url, Optional<Map<String, String>> tags
        Movie movie =
            new Movie(
                mediaId,
                title,
        ...
        return new MovieProgress(
            rec.getProgressId(), movie, movieProgress, rec.getFavorite(), rec.getFinished());
      case Series.TYPE:
        // int id,String title,Set<String> genre,Map<String, Integer> seasons,String platform,
        // Optional<String> url, Optional<LocalDate> releaseYear, Optional<LocalDate>
        // endYear,Optional<Map<String, String>> tags
        Series series =
            new Series(
                mediaId,
                title,
                ...
                platform,
                optionalUrl,
                Optional.of(LocalDate.of(releaseYear, 1, 1)),
                Optional.of(LocalDate.of(endYear, 1, 1)),
                Optional.of(tags));
        ...
        //        Map<String, Integer> progress, // Progress map (e.g., "Season 1" -> 5, meaning 5
        // episodes watched)
        //        boolean favorite
        return new SeriesProgress(
            rec.getProgressId(), rec.getFinished(), series, seriesProgress, rec.getFavorite());
      case VideoGame.TYPE:
        // int id, String title, String platform, Set<String> genre, String publisher, String
        // developer, int year, Optional<Map<String, String>> tags
        VideoGame videoGame =
            new VideoGame(
                mediaId,
                title,
  ...
  private Map<String, Integer> parseProgress(JSONB progress) {
    ...
    if (!jsonData.isBlank()) {
      ...
      try {
        progressMap =
            objectMapper.readValue(jsonData, new TypeReference<Map<String, Integer>>() {});
      } catch (JsonProcessingException e) {
        throw new RuntimeException(e);
      }
    }
