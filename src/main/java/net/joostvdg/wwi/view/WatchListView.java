...
public class WatchListView extends VerticalLayout {
  ...
  private final transient Logger logger = LoggerFactory.getLogger(WatchListView.class);
  ...
  private final Grid<WatchList> sharedWatchlistGrid;

  private final TextField nameField = new TextField("Name");
  private final TextArea descriptionField = new TextArea("Description");

  public WatchListView(
      WatchlistService watchlistService, ProgressService progressService, UserService userService) {
    this.watchlistService = watchlistService;
    this.progressService = progressService;
    this.userService = userService;

    List<WatchList> sharedWatchlists =
        watchlistService.findSharedWith(userService.getLoggedInUser());
    ListDataProvider<WatchList> sharedWatchlistsProvider = new ListDataProvider<>(sharedWatchlists);
    ...
    grid.addComponentColumn(
            watchlist -> {
              Button shareButton =
                  new Button(
                      "Share", e -> openShareDialog(watchlist, userService, watchlistService));
              return new HorizontalLayout(shareButton);
            })
        .setHeader("Actions");

    Button saveButton = new Button("Save Watchlist");
    add(
        new HorizontalLayout(nameField, descriptionField),
        saveButton,
        new VerticalLayout(new H1("My Watch Lists"), grid));
  ...
  private void configureSharedWatchlistGrid() {
    ...
    sharedWatchlistGrid
        .addColumn(watchList -> watchList.getItems().size())
        .setHeader("Media Items")
        .setSortable(true);
    sharedWatchlistGrid
        .addColumn(
            watchList ->
                watchList.getOwner().username()
                    + " ("
  ...
  private void openSharedWatchListDetails(WatchList watchList) {
    ...
    logger.info(
        "Found {} progresses and {} items in watchlist",
        progressSet.size(),
    ...
    for (Media item : watchList.getItems()) {
      for (Progress progress : progressSet) {
        logger.info(
            "Checking progress for media: {} and progress: {}" + item.getId(),
            progress.getMedia().getId());

        if (progress.getMedia().getId() == item.getId()) {
          items.add(
              new SharedWatchListItem(
    ...
    itemGrid
        .addColumn(SharedWatchListItem::title)
        .setHeader("Title")
        .setSortable(true)
        .setAutoWidth(true);
    itemGrid
        .addColumn(SharedWatchListItem::mediaType)
        .setHeader("Type")
        .setSortable(true)
        .setAutoWidth(true);
    itemGrid
        .addColumn(SharedWatchListItem::platform)
        .setHeader("Platform")
        .setSortable(true)
        .setAutoWidth(true);
    itemGrid
        .addColumn(SharedWatchListItem::progressSummary)
        .setHeader("Progress")
        .setSortable(true)
        .setAutoWidth(true);
  ...
  private Set<Progress> getProgressForWatchlist(WatchList watchList) {
    return progressService.getProgressForUserAndMedia(
        watchList.getOwner().id(), watchList.getItems());
  }

  private void openShareDialog(
      WatchList watchlist, UserService userService, WatchlistService watchlistService) {
    ...
    shareDialog.add(
        new VerticalLayout(userSearch, new HorizontalLayout(shareButton, cancelButton)));
    shareDialog.open();
  }
  ...
  private void configureGrid() {
    ...
    grid.addColumn(watchList -> watchList.getItems().size())
        .setHeader("Media Items")
        .setSortable(true);
    grid.addColumn(watchList -> watchList.isFavorite() ? "Yes" : "No")
        .setHeader("Favorite")
        .setSortable(true);
  ...
  private void saveWatchList() {
    ...
    Set<Media> items =
        new HashSet<>(); // Empty for now, but you can add Series, Movie, or VideoGame here
  ...
  private void openWatchListDetails(WatchList watchList) {
    ...
    getUI()
        .ifPresent(ui -> ui.navigate(WatchListDetailsView.class, Long.toString(watchList.getId())));
  }
}
