...
public class WatchListDetailsView extends VerticalLayout implements HasUrlParameter<String> {
  ...
  private final FormLayout watchListDetailsForm =
      new FormLayout(); // Section 1: Watchlist properties
  private final Grid<Progress> progressGrid = new Grid<>(Progress.class, false);

  public WatchListDetailsView(
  ...
  private void configureProgressGrid() {
    ...
    progressGrid
        .addColumn(progress -> progress.getMedia().getTitle())
        .setHeader(Labels.TITLE)
        .setSortable(true)
        .setAutoWidth(true);

    progressGrid
        .addColumn(progress -> progress.getMedia().getPlatform())
    ...
    progressGrid
        .addColumn(
            progress ->
                progress
                    .getProgress()
                    .toString()) // Display progress details (e.g., episodes watched)
        .setHeader(Labels.PROGRESS)
        .setAutoWidth(true);

    progressGrid
        .addColumn(progress -> progress.isFinished() ? "Yes" : "No")
        .setHeader(Labels.FINISHED)
        .setSortable(true)
        .setAutoWidth(true);

    progressGrid
        .addColumn(progress -> progress.isFavorite() ? "Yes" : "No")
    ...
    isFavoriteFilter.addValueChangeListener(
        event -> applyFilters(isFavoriteFilter, isFinishedFilter));
    isFinishedFilter.addValueChangeListener(
        event -> applyFilters(isFavoriteFilter, isFinishedFilter));
  ...
  private void openViewDialog(Progress progress) {
    ...
    if (progress.getMedia() instanceof Movie movie) {
      formLayout.addFormItem(
          new Span(String.valueOf(movie.durationInMinutes())), "Duration (in minutes)");
    }
  ...
  private void openEditProgressDialog(Progress progress) {
    ...
    Map<String, Integer> progressMap =
        new HashMap<>(progress.getProgress()); // Editable progress map
    ...
    if (progress.getMedia() instanceof Series series) {
      ...
      for (Map.Entry<String, Integer> entry : series.seasons().entrySet()) {
        ...
        seasonField.addValueChangeListener(
            e -> progressMap.put(season, Integer.parseInt(e.getValue())));
      }
      ...
      mainProgressField.addValueChangeListener(
          e -> progressMap.put("main", Integer.parseInt(e.getValue())));
      ...
      sideProgressField.addValueChangeListener(
          e -> progressMap.put("side", Integer.parseInt(e.getValue())));
      ...
      collectiblesProgressField.addValueChangeListener(
          e -> progressMap.put("collectibles", Integer.parseInt(e.getValue())));
      ...
      mainProgressField.addValueChangeListener(
          e -> progressMap.put("Minutes Watched", Integer.parseInt(e.getValue())));
    }
              ...
              if (progress.getMedia() instanceof Series) {
                updatedProgress =
                    new SeriesProgress(
                        ...
                        (Series) progress.getMedia(),
                        progressMap,
                        favoriteCheckbox.getValue());
              } else if (progress instanceof VideoGameProgress) {
                updatedProgress =
                    new VideoGameProgress(
                        progress.getId(),
                        progressMap,
                        finishedCheckbox.getValue(),
                        (VideoGame) progress.getMedia(),
                        favoriteCheckbox.getValue());
              } else if (progress.getMedia() instanceof Movie) {
                updatedProgress =
                    new MovieProgress(
                        progress.getId(),
                        (Movie) progress.getMedia(),
                        progressMap,
                        favoriteCheckbox.getValue(),
                        finishedCheckbox.getValue());
              }
  ...
  private void populateProgressGrid(Grid<Progress> progressGrid) {
    Set<Progress> progresses =
        progressService.getProgressForUserAndMedia(
            currentWatchList.getOwner().id(), currentWatchList.getItems());
        ...
        new Button(
            ...
            event -> {
              try {
                ...
                progressService.createVideoGameProgressForUser(
                    currentWatchList.getOwner(), newVideoGame);
